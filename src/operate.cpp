...
bool Operate::isTurboModeSupported() const
{
    if (msiEcHelper.hasShiftMode())
    {
        // Try to detect if turbo mode is supported
        shift_mode currentMode = msiEcHelper.getShiftMode();

        // Attempt to set turbo mode temporarily
        msiEcHelper.setShiftMode(shift_mode::turbo_mode);
        if (updateEcData())
        {
            shift_mode newMode = msiEcHelper.getShiftMode();

            // Restore original mode
            msiEcHelper.setShiftMode(currentMode);

            // If the mode was successfully changed to turbo, then it's supported
            return newMode == shift_mode::turbo_mode;
        }

        // Restore original mode if update failed
        msiEcHelper.setShiftMode(currentMode);
        return false;
    }
    return false;
...
bool Operate::isEcoModeSupported() const
{
    if (msiEcHelper.hasShiftMode())
    {
        // Try to detect if eco mode is supported
        shift_mode currentMode = msiEcHelper.getShiftMode();

        // Attempt to set eco mode temporarily
        msiEcHelper.setShiftMode(shift_mode::eco_mode);
        if (updateEcData())
        {
            shift_mode newMode = msiEcHelper.getShiftMode();
            ...
            // If the mode was successfully changed to eco, then it's supported
            return newMode == shift_mode::eco_mode;
        }
...
bool Operate::isSilentModeSupported() const
{
    if (msiEcHelper.hasFanMode())
    {
        // Try to detect if silent mode is supported
        fan_mode currentMode = msiEcHelper.getFanMode();

        // Attempt to set silent mode temporarily
        msiEcHelper.setFanMode(fan_mode::silent_fan_mode);
        if (updateEcData())
        {
            fan_mode newMode = msiEcHelper.getFanMode();

            // Restore original mode
            msiEcHelper.setFanMode(currentMode);

            // If the mode was successfully changed to silent, then it's supported
            return newMode == fan_mode::silent_fan_mode;
        }
    ...
    return helper.getValue(fanModeAddress) == fanModeSilent ||
           helper.getValue(fanModeAddress) == fanModeAuto;
}

bool Operate::isPerformanceModeSupported() const
{
    return isTurboModeSupported() ||
           (msiEcHelper.hasShiftMode() && msiEcHelper.getShiftMode() == shift_mode::sport_mode);
}

bool Operate::isSuperBatteryModeSupported() const
{
    return isEcoModeSupported() || msiEcHelper.hasSuperBattery();
}
